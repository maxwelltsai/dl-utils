...
def _process_image(filename, coder):
  ...
  width = image.shape[1]
  assert image.shape[2] == 3
  print('Decoding %s, resultion = (%d * %d)' % (os.path.basename(filename), height, width))
...
def _process_dataset(filenames, synsets, labels, output_directory, output_filename):
  """Processes and saves list of images as TFRecords.
  Args:
    filenames: list of strings; each string is a path to an image file
    synsets: list of strings; each string is a unique WordNet ID
    ...
    output_filename: string; the filename of the TFRecord
  Returns:
  ...
  files = []
  output_file = os.path.join(output_directory, '%s' % output_filename)
  _process_image_files_batch(coder, output_file, filenames, synsets, labels)
...
def convert_to_tf_records(raw_data_dir):
  ...
  training_records = _process_dataset(training_files, training_synsets, labels, 
                                      FLAGS.local_scratch_dir, 'img.tfrecords')
...
def main(argv):  # pylint: disable=unused-argument
  ...
  if FLAGS.local_scratch_dir is None:
    FLAGS.local_scratch_dir = os.getcwd()
  ...
  training_records = convert_to_tf_records(raw_data_dir)
